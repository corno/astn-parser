...
import { IPreTokenStreamConsumer } from "./IPreTokenStreamConsumer"
import { getEndLocationFromRange } from "./getEndLocationFromRange"
import { WrappedStringType } from "./PreToken"

function createRangeFromLocations(start: astn.Location, end: astn.Location): astn.Range {
    return {
        start: start,
...
function createRangeFromSingleLocation(location: astn.Location): astn.Range {
    return {
        start: location,
        length: 0,
...
export function printTokenizer2Error(error: astn.Tokenizer2Error): string {
    return error[0]
}
...
export function createTokenizer2(
    ...
    onError2: ($: {
        error: astn.Tokenizer2Error
        range: astn.Range
    }) => void,
): IPreTokenStreamConsumer {

    function onError(error: astn.Tokenizer2Error, range: astn.Range) {
        onError2({
            error: error,
            range: range,
        })
    ...
    function setCurrentToken(contextType: CurrentToken, range: astn.Range) {
        if (currentToken[0] !== "none") {
            onError(["unexpected start of token", {}], range)
        }
        currentToken = contextType
    }
    function unsetCurrentToken(range: astn.Range) {
        if (currentToken[0] === "none") {
        ...
        onData: (data) => {
            switch (data.type[0]) {
                ...
                case "line comment end": {
                    const $ = data.type[1]
                    function onLineCommentEnd(location: astn.Location) {
                ...
                case "newline": {
                    const $ = data.type[1]
                    function onNewLine(_range: astn.Range, _tokenString: string) {
                ...
                case "wrapped string begin": {
                    const $ = data.type[1]
                    indentationState.setLineDirty()
                    function onWrappedStringBegin(begin: astn.Range, quote: WrappedStringType) {
                        setCurrentToken(
                            ["wrapped string", {
                                wrappedStringNode: "",
                                start: begin,
                ...
                case "wrapped string end": {
                    const $ = data.type[1]
                    function onWrappedStringEnd(end: astn.Range, wrapper: string | null) {
                ...
                case "non wrapped string begin": {
                    const $ = data.type[1]
                    function onNonWrappedStringBegin(location: astn.Location) {
                ...
                case "non wrapped string end": {
                    const $ = data.type[1]
                    function onNonWrappedStringEnd(location: astn.Location) {
                ...
                case "whitespace begin": {
                    const $ = data.type[1]
                    function onWhitespaceBegin(location: astn.Location) {
                ...
                case "whitespace end": {
                    const $ = data.type[1]
                    function onWhitespaceEnd(location: astn.Location) {
